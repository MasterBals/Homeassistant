...
const META = {
  karton: { label: "Karton", icon: "mdi:package-variant-closed", asset: `${ASSET_BASE}/karton.svg`, color: "#b7791f", accent: "#fff3d8" },
  papier: { label: "Papier", icon: "mdi:newspaper-variant-multiple-outline", asset: `${ASSET_BASE}/papier.svg`, color: "#2563eb", accent: "#e0ecff" },
  kompost: { label: "Kompost", icon: "mdi:leaf-circle-outline", asset: `${ASSET_BASE}/kompost.svg`, color: "#15803d", accent: "#def7e6" },
  kehricht: { label: "Kehricht", icon: "mdi:trash-can-outline", asset: `${ASSET_BASE}/kehricht.svg`, color: "#475569", accent: "#e9eef5" },
...
class ChurAbfallCard extends HTMLElement {
  ...
  render() {
    ...
    this.innerHTML = `<ha-card><style>
      ...
      .hero{display:grid;grid-template-columns:112px 1fr;gap:16px;align-items:center;padding:18px;border-radius:18px;background:linear-gradient(135deg,var(--a),var(--card-background-color));border:1px solid color-mix(in srgb,var(--c) 32%,var(--divider-color));overflow:hidden}.hero-art{width:106px;height:96px;display:grid;place-items:center;filter:drop-shadow(0 12px 12px color-mix(in srgb,var(--c) 18%,transparent))}.art{width:100%;height:100%;object-fit:contain}.fallback{color:var(--c);--mdc-icon-size:42px}.animated .hero-img{animation:floaty 3s ease-in-out infinite}.urgent.animated .hero-art{animation:attention 1.6s ease-in-out infinite}.eyebrow{font-size:.78rem;text-transform:uppercase;color:var(--secondary-text-color);font-weight:700}.hero-name{font-size:1.25rem;font-weight:800}.hero-date{font-size:2rem;font-weight:900;line-height:1.05;color:var(--c)}.street{margin-top:6px;color:var(--secondary-text-color);font-size:.9rem}
      .tiles{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;margin-top:12px}.tile{display:grid;grid-template-columns:52px 1fr;align-items:center;gap:10px;padding:10px;border-radius:14px;background:var(--card-background-color);border:1px solid var(--divider-color)}.tile.current{border-color:color-mix(in srgb,var(--c) 46%,var(--divider-color));background:var(--a)}.tile-art{width:48px;height:42px;display:grid;place-items:center;filter:drop-shadow(0 6px 6px color-mix(in srgb,var(--c) 12%,transparent))}.animated .tile.current .tile-img{animation:floaty 3s ease-in-out infinite}.tile-name{font-weight:700;font-size:.92rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.tile-date{font-size:.86rem;color:var(--secondary-text-color);margin-top:1px}.empty{display:flex;align-items:center;gap:14px;padding:18px;color:var(--secondary-text-color)}.empty ha-icon{--mdc-icon-size:34px;color:var(--primary-color)}
      @keyframes floaty{0%,100%{transform:translateY(0) rotate(0)}50%{transform:translateY(-4px) rotate(-2deg)}}@keyframes attention{0%,100%{transform:scale(1)}50%{transform:scale(1.06)}}@media(max-width:460px){.wrap{padding:14px}.hero{grid-template-columns:78px 1fr;padding:14px}.hero-art{width:74px;height:68px}.hero-date{font-size:1.55rem}.tiles{grid-template-columns:1fr}}
